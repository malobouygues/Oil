...
- Calibrates SABR parameters *(α, ρ, ν)* with β fixed

## Model choice
WTI options are options on futures, so the standard model is Black76.

## SABR (smile model)
SABR produces an implied-vol smile via the approximation of Hagan:
...
- `src/models/implied_vol.py` – implied vol solver
...
## Sanity check (on a single expiry)
Below is a pricing check on a near ATM option: market premium is almost equal to model PV, and greeks are produced.
